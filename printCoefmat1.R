printCoefmat1 <- function(x, digits = max(3L, getOption("digits") - 2L),
  ...
  if (length(zap.ind)) {
    ...
    Cf[, zap.ind] <- format(round(xm[, zap.ind, drop = FALSE],
                                  max(1L, digits)), digits = digits)
  }

  if (length(nonzap.cs.ind <- setdiff(cs.ind, zap.ind))) {
    nonzap.cs <- xm[, nonzap.cs.ind, drop = FALSE]
    ...
    if (any(ia <- is.finite(acs))) {
      ...
      Cf[, nonzap.cs.ind] <- format(round(nonzap.cs, max(1L, digits -
...
# x = c(3.760667e-07, 0.4992263, 0.6009902)
# zapsmall(x, digits = 4)
